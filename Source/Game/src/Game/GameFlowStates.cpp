...
void cStateTitleScreen::VOnEnter(cGame *pGame)
{
	if (pGame->m_pHumanView->m_pAppWindowControl != NULL)
	{
		...
		def.pFont = IFontManager::GetInstance()->VGetFont("arial.fnt"); // JokerMan
...
void cStateMenuScreen::VOnEnter(cGame *pGame)
{
	if (pGame->m_pHumanView->m_pAppWindowControl != NULL)
	{
		...
		buttonDef.labelControlDef.pFont = IFontManager::GetInstance()->VGetFont("arial.fnt"); // Vladimir Script
		...
		pGame->m_pHumanView->m_pAppWindowControl->VAddChildControl(shared_ptr<IBaseControl>(pSinglePlayerButton));
		pSinglePlayerButton->VSetPosition(cVector2(311.f, 310.f));
		...
		pGame->m_pHumanView->m_pAppWindowControl->VAddChildControl(shared_ptr<IBaseControl>(pMultiPlayerButton));
		pMultiPlayerButton->VSetPosition(cVector2(311.f, 366.f));
		pMultiPlayerButton->VSetSize(cVector2(pSinglePlayerButton->VGetWidth(), pSinglePlayerButton->VGetHeight()));
		...
		pGame->m_pHumanView->m_pAppWindowControl->VAddChildControl(shared_ptr<IBaseControl>(pQuitButton));
		pQuitButton->VSetPosition(cVector2(311.f, 422.f));
		pQuitButton->VSetSize(cVector2(pSinglePlayerButton->VGetWidth(), pSinglePlayerButton->VGetHeight()));
